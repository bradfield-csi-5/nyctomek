...
void flush_cache() {
  int size = 8 * 1024 * 1024; // 8MB to clear out L3
  char *b = malloc(size);
...
int main(int argc, char *argv[]) {
  ...
  double **C_preheat = matrix_alloc(n, n);
  ...
  // Call once up front.  Otherwise, the second invocation
  // always seems to perform better, even when calling the
  // naive implementation twice in a row.
  matrix_multiply(C_preheat, A, B, n, n, n);
